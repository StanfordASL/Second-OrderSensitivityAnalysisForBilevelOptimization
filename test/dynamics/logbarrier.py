...
def barrier(A, x, b, s):
    return torch.sum(-torch.log(-s * (A @ x - b)) / s)


def dbarrier(A, x, b, s):
    return -A.transpose(-1, -2) @ (1.0 / (A @ x - b) / s)
...
if __name__ == "__main__":
    ...
    X, Y = visualize_landscape(
        lambda x: loss_fn_(opt_fn_(x), x), x_hist, 100, log=False
    )
